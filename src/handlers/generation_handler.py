...
# smart-gen run (which spends the user's own API key), but it is an
...
def _plan_pause_exact_decision(message: str) -> Optional[str]:
    ...
    classifier verdict: the classifier stamps short acknowledgments
...
# confirmation copy honestly (from-scratch wording reads wrong when the user
# just pasted a traceback / asked for a fix). Strong, unambiguous
# fix/error vocabulary only — plus HTTP 4xx/5xx status codes.
#
# NOT matched against the classifier's refined instructions: those are
...
# and would flip a from-scratch build to the fix copy.
...
def _smart_gen_project_has_app(session: Session, project_id: Optional[str]) -> bool:
    ...
    # on those is a regression, not safety: "fix my app" would lose the
    # SMART-GEN FOLLOW-UP signal and be answered as a help question instead of
    # arming a fix.
    #
...
def _looks_like_fix_request(
    ...
    Without the project gate, a first message in a brand-new project would get
    the fix copy ("I'll update your existing app") because a run in a PREVIOUS
    project of the same session was still within the 30-min window.

    With the project gate necessary, the two signals only choose WHICH
    ...
    * **Fresh run for this project** (within 30 min) — the next smart-gen
      ...
      modify (which reuses the last run as its base), whether or not the
      user used fix words.
...
def _build_smart_gen_confirmation(
    ...
    *,
    ...
    run would otherwise begin silently. The confirm/cancel phrases
    ...
    fix-framing + a "Fix it" button (from-scratch wording reads wrong when
    the user only wants a fix). Both branches keep the
    ...
    return {
        ...
        "message": message,
        # No Cancel action — the proposition offers
...
def _get_classification_from_cache_or_classify(session, request):
    ...
    # (uncached; one classifier-tier call).
...
# would still trigger the "which dialect?" config prompt.
...
def handle_pending_plan_generation_confirmation(session: Session) -> bool:
    ...
    short answers — a literal "ok" can be stamped ``decline_intent`` —
...
def _handle_smart_generator_result(
    ...
    Records the outcome (success, cost, failure reason) in conversation
    memory so the agent can follow up ("why did it fail?"), and replies with
    an outcome-aware message + suggested next steps.
    """
    ...
    if session is not None:
        if ok:
            ...
            # project the user opens in the same session.
            session.set(
                LAST_SMART_GEN_PROJECT_ID,
...
def handle_generation_request(session: Session, request: AssistantRequest) -> Dict[str, Any]:
    ...
    request. Our job here is the SUB-routing: Spec-Driven Agent (for any
    ...
    # Past-generation QUESTION guard: after a run finished, "What we
    # generated" can classify as generation_intent and re-arm the whole
    # confirmation instead of being answered. A past-tense/interrogative
    # reference to the finished run — with no future-directed word — is a
    # question about the outcome, never a new
    ...
    if _gh_ref is not None:
        ...
        # "yes" can never spend against a stale pre-import run.
        _clear_pending_smart_gen(session)
        _clear_pending_state(session)
    ...
    if has_pending_smart_gen and smart_gen_decision is None:
        ...
        # (an LLM can read e.g. "fast", meant for the GUI choice, as an eager
        # confirm).
    ...
    if smart_gen_decision == "confirm":
        ...
        # other path stashes + asks first (the run spends the
        # user's own API key).
        from handlers.smart_generation_handler import GenerationClassification
        stashed_instructions = session.get(PENDING_SMART_GEN_INSTRUCTIONS) or ""
        stashed_provider = session.get(PENDING_SMART_GEN_PROVIDER) or "anthropic"
    ...
    # button's label instead of clicking it. The raw label would re-classify as
    # a fresh smart-gen request and re-show the mismatch question in a loop
    # (and, below, clear the stashed run as a "different request"). Treat any
    # label alias as the button press: dispatch the stashed rebuild prompt to
    # the modeling path — the create choke point
    ...
    # never be re-litigated there (the classifier can stamp a literal "ok" as
    # decline_intent, and the opt-out would cancel a confirmed run).
    ...
    # database" while mid-Django-config, or escalating to the Spec-Driven Agent.
    ...
    if _use_pending:
        ...
        # "ok"/"yes" must not override the deterministic answer.
    ...
    if _use_pending:
        ...
        if classification.route == "smart":
            ...
            # Never fire directly: the Spec-Driven Agent spends the user's
            # own API key, so stash + ask for explicit confirmation.
            # Pass the user's own message so the confirmation copy can tell a
            # fix/modify apart from a first build.
