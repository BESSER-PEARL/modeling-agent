...
_SYSTEM_PROMPT = (
    ...
    "conversation, classify 'add X' by its normal meaning. NOT a "
    "follow-up: a message that simply NAMES a BESSER built-in generator "
    "with no feature words — 'generate a rest api', 'generate sql', "
    "'generate the backend' — is a fresh deterministic generator request "
    "(rules 3c/4), even minutes after a smart run.\n\n"
    ...
    "3c. rest_api / backend produce a REAL, runnable FastAPI service on the "
    "DETERMINISTIC path — treat them EXACTLY like sql in 3b, NOT as 'a real "
    "app' that needs smart. A bare 'generate a rest api', 'generate the "
    "backend', 'give me a rest api for my model', 'expose my model as a rest "
    "api' is 'deterministic' with generator_type='rest_api' (or 'backend'); "
    "the words 'api'/'rest'/'backend' do NOT make it smart. Escalate to "
    "'smart' ONLY when the request adds behavior the template can't express — "
    "authentication / login / sign-up, roles / permissions / access control, "
    "JWT / OAuth, Docker / containers, a front-end or UI, deployment, business "
    "rules. Examples (all 'deterministic', rest_api): 'generate a rest api', "
    "'generate the backend', 'rest api from my model'. Examples ('smart'): 'a "
    "rest api with authentication', 'a dockerized backend', 'a backend with "
    "user login and roles'. CRITICAL: this rule holds EVEN right after a "
    "smart / Spec-Driven generation (RECENT SMART GENERATION line present) — "
    "a bare request naming a BESSER built-in is a fresh deterministic "
    "generator run, NOT a follow-up to the smart app.\n"
    ...
    try:
        if session.get(PENDING_GUI_CHOICE):
            return {
                "kind": "gui_choice",
                "question": (
                    "How should I create your screens — 1 basic CRUD pages "
                    "(one page per class), or 2 experimental AI design?"
                ),
